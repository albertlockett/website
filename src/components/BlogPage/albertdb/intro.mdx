...
I also had no experience with database development, but I had also recently
  finished reading _Database Internals_ by Alex Petrov (one of the best books
  I've ever read).
I was looking for a project to help solidify the concepts.

To wit, you're reading an article about a database written in rust by someone
...
_Reminder: Option in Rust is an enum with two values, `None` and `Some(<T>)`.
...
This type is like an ogre, it has layers that must be unpacked to be understood.

Firstly, if the relationship doesn't exist, the option will be `None`.
...
The next layer is the `Arc` type which is a reference counted pointer that can
  be passed between threads (rc = reference counted, A = asyncrhonous).
...
_TODO write the rest of this section_
