...
Also was mentioned, it more/less takes the form of base64 encoded JSON.
I sat "more/less" because it really has some additional structure

```
TODO put a picture here of the real format or something
```
...
Signing the token is a confusing as fuck topic
  but luckily there's a good book on it.
If you learn nothing else,
  here's the rundown...

Basically the signure is this:
```
signature = base64(signingAlg(`${header}.${payload}`))
```
...
It uses a secret, which the token profider (the
  ...
  the token.
This is the most common signing algorithm and often
...
RS256 aka RSA + SHA256 is an asymettic signing 
...
But I figured that - what use is writing all this
  shit about oauth if no one knew what to do with
  the output or why they're able to do it.

Anyway, the code to do all this signing and funky shit
  is confusing as hell so it's best to use a blirary for it.
...
```
// TODO make this look like fetch
...
Something about a dog's breakfast
