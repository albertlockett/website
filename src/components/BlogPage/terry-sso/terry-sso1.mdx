...
```
TODO add something about here being where we're redirected and something
about how the session is setup
```
```
TODO something about session and the challenge format
```
...
Hence, we also submit the _session_id_, which is just some id that can be used to
...
```
TODO, need to talk about the error handling here as well.
TODO, refactor the backend of part 1 to return the session id
```
